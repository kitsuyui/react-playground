...
export type DigitalClockProps = React.ComponentPropsWithoutRef<'span'> & {
  timezone: string
  date: Date
}
 ...
 * @returns
 */
export const DigitalClock = (props: DigitalClockProps): React.JSX.Element => {
  const { timezone, date, ...spanProps } = props
  const dateStr = dateToDigital(date, timezone)
  return (
    <span {...spanProps}>{dateStr}</span>
  )
}
