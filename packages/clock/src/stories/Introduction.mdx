...
2. The state of the clock (`ClockContextProvider`, `ClockContext`, etc.)
...
`ClockContextProvider` is a component that manages the state of the clock.
The `refreshInterval` property is the interval at which the clock is updated.
`AnalogClock` and `DigitalClock` are components that display the clock. These components are independent of the state of the clock and accept ordinary element props for their outer wrapper.
